...
function getCamelCasedWords(str: string): string[] {
  return str.match(/[a-zA-Z]+[A-Z][a-z]*/g) || [];
}
...
function isPermutation(list: number[], perms: number[]): boolean {
  return list.length === perms.length && list.every(num => perms.includes(num));
}
...
function pow(x: number, n: number): number {
  return Array.from({ length: n }, () => x).reduce((acc, curr) => acc * curr, 1);
}
...
function digitsNumberValueInBase(b: number, bDigits: number[]): number {
  return bDigits.reduce((acc, digit, index) => acc + digit * Math.pow(b, index), 0);
}
...
function bStringValue(bString: string, b: number): number {
  const charToDigit = (char: string): number => {
    ...
    return (digit >= 48 && digit <= 57) ? digit - 48 :
      (digit >= 65 && digit <= 90) ? digit - 55 :
        (digit >= 97 && digit <= 122) ? digit - 87 :
          (() => { throw new Error(`Invalid character in string: ${char}`) })();
  };

  return bString.split('').reduce((result, char) => result * b + charToDigit(char), 0);
}
if (true) {
...
function nTuples<T>(ls: T[], n: number): T[][] {
  return Array.from({ length: ls.length / n }, (_, i) => ls.slice(i * n, (i + 1) * n));
}
...
function e(n: number): number {
  ...
  const eTerm = (k: number): number => 1 / factorial(k);

  const eSum = (current: number, k: number): number => {
    return k > n ? current : eSum(current + eTerm(k), k + 1);
  };
